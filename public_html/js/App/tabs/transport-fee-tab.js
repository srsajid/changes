 ...
 */
var _tf = App.tabs.transport_fee = new TableTab("transport_fee", "Transport fee", App.baseUrl + "transport/load-table");
...
_tf.takeFee = function (id){
    var _self = this;
    var title = "Transport fee payment";
    util.editPopup(title, App.baseUrl+ "transport/transport-fee-form", {
        data: {id: id},
        after_load: function() {
            var dom = this;
            ...
            function loadNextStep() {
                dom.loader();
                util.ajax({
                    url: App.baseUrl+ "transport/transport-fee-next",
                    dataType: 'json',
                    ...
                    success: function(resp, status, xhr) {
                       ...
                       resp.find("[name=monthCount], [name=fine], [name=discount]").on("change", function() {
                          ...
                          var perMonthFee = dom.find(".per-month-transport").text().trim();
                          ...
                          var total = noOfMonth * perMonthFee - (discount ? discount : 0) + parseFloat(fine ? fine : 0);
                           resp.find(".total-transport").text(total);
                       });
